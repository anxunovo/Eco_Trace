...
console.log(`Inserting ${DB_CARBON_COEFFICIENTS.length} carbon coefficients...`);
...
const insertSQL = `INSERT OR IGNORE INTO listings (
  id, owner_id, title, description, category, trade_mode, price, \`condition\`,
  campus, location_text, contact_method, contact_value, images,
  estimated_weight_kg, estimated_carbon_saved_kg, ai_confidence, ai_assumptions,
  is_food, food_info, tags, interested_count, status, created_at
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
...
console.log(`Done: ${listings.length} listings`);
